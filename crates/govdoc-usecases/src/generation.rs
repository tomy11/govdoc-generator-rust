...
pub const STRUCTURING_SYSTEM_PROMPT: &str = "คุณคือผู้ช่วยแยกข้อมูลจากข้อความหนังสือราชการไทยให้อยู่ในรูป JSON ตาม schema ที่กำหนด\nดึงข้อมูลจากข้อความที่ให้เท่านั้น ห้ามแต่งเติมข้อมูลที่ไม่ปรากฏในข้อความ\nถ้าฟิลด์ใดไม่พบ ให้ใส่ค่าว่างหรือรายการว่าง\nตอบเป็น JSON ตาม schema เท่านั้น";
...
/// Parse free OCR text into a validated, structured document of `doc_type`.
///
/// Used during ingestion so stored examples are schema-shaped rather than raw
/// text. Returns a validation error if the model output does not satisfy the
/// document schema, letting callers fall back to storing the raw text.
pub async fn structure_document_from_text(
    doc_type: &DocType,
    raw_text: &str,
    llm: &dyn LlmProvider,
) -> Result<Value, GenerationError> {
    let schema = schema_for_doc_type(doc_type);
    let prompt = build_structuring_prompt(doc_type, raw_text, &schema);
    let draft = llm
        .complete_json(STRUCTURING_SYSTEM_PROMPT, &prompt, schema, 8192)
        .await?;
    validate_doc_json(doc_type, draft)
}

fn build_structuring_prompt(doc_type: &DocType, raw_text: &str, schema: &Value) -> String {
    [
        format!(
            "จงแยกข้อมูลจากข้อความหนังสือราชการประเภท {} ต่อไปนี้ให้อยู่ในรูป JSON",
            doc_type.as_thai()
        ),
        String::new(),
        "ข้อความ (จาก OCR):".to_string(),
        raw_text.to_string(),
        String::new(),
        "Schema JSON ที่ต้องเอาต์พุต:".to_string(),
...
mod tests {
    ...
    impl LlmProvider for StubLlm {
        async fn complete(&self, _: &str, _: &str, _: usize) -> anyhow::Result<String> {
            Ok(String::new())
        }
    ...
    #[tokio::test]
    async fn structures_text_into_validated_schema() {
        let doc = json!({
            "doc_type": "ภายนอก",
            "number": "ศธ 0000/0001",
            "agency": "โรงเรียนตัวอย่าง",
            "date": "1 มกราคม 2569",
            "subject": "ขอเชิญประชุม",
            "recipient": "ผู้อำนวยการเขต",
            "salutation": "เรียน",
            "reference": [],
            "enclosure": [],
            "body": ["ย่อหน้าที่ได้จากการแยกข้อความ"],
            "closing": "ขอแสดงความนับถือ",
            "signer_name": "นายสมชาย",
            "signer_position": "ผู้อำนวยการ"
        });
        let llm = StubLlm(doc.clone());

        let out = structure_document_from_text(&DocType::External, "ข้อความดิบจาก OCR", &llm)
            .await
            .unwrap();

        assert_eq!(out["subject"], "ขอเชิญประชุม");
        assert_eq!(out["body"][0], "ย่อหน้าที่ได้จากการแยกข้อความ");
    }

    #[tokio::test]
    async fn structuring_rejects_invalid_schema() {
        // Missing required fields -> validation error, so callers can fall back.
        let llm = StubLlm(json!({ "subject": "ไม่ครบ" }));
        let result = structure_document_from_text(&DocType::External, "ข้อความ", &llm).await;
